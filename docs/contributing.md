...
[pull requests](https://github.com/munich-quantum-toolkit/qcec/pulls).
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/qcec/issues>.
...
[GitHub Actions page](https://github.com/munich-quantum-toolkit/qcec/actions/workflows/ci.yml).
...
you can pass {code}`-DBUILD_MQT_QCEC_TESTS=OFF` to the CMake
configure step.
:::

Our CI pipeline on GitHub also collects code coverage information
and uploads it to
[Codecov](https://codecov.io/gh/munich-quantum-toolkit/qcec).
...
pass {code}`-DBUILD_MQT_QCEC_BINDINGS=ON` to the CMake configure
step.
...
{code}`python/mqt/qcec` directory.
...
MQT QCEC adheres to [Semantic Versioning],
...
Use it to explain how users may need to adapt their usage of MQT QCEC,
...
When it is time to release a new version of MQT QCEC,
...
changes for MQT QCEC, you can remove the {code}`minor` or {code}`major`
label from the PR.
...
[Releases page](https://github.com/munich-quantum-toolkit/qcec/releases) on
